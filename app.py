...
if not st.session_state.logado:
    ...
    if st.session_state.is_superadmin:
        with st.sidebar.form("form_criar_tenant"):
            st.write("**👑 Criar Novo Grupo**")
            nome_b = st.text_input("Nome da Liga")
            admin_b = st.text_input("E-mail do Admin").lower().strip()
            if st.sidebar.form_submit_button("Criar Liga", use_container_width=True):
                ...
                st.sidebar.success(f"Liga criada!")
                st.rerun()
    ...
    if meus_grupos:
        # UX Mobile: c1, c2, c3 adaptam-se para stacking no telemóvel
        ...
        for idx, grupo in enumerate(meus_grupos):
            with [c1, c2, c3][idx % 3]:
                ...
                if st.button("Entrar", key=f"lk_{grupo['id_bolao']}", use_container_width=True):
                    ...
                    st.rerun()
    else: st.warning("Peça ao Admin para autorizar o seu e-mail num grupo.")
...
# ==========================================
    ...
    st.sidebar.title(f"🌍 {st.session_state.bolao_ativo_nome}")
    if st.sidebar.button("🏠 Voltar aos Grupos", use_container_width=True):
        st.session_state.update(bolao_ativo_id=None); st.rerun()
        
    st.sidebar.divider()
    menu_opcoes = ["Fazer Palpites de Jogos", "Bônus 1: Videntes dos Grupos", "Bônus 2: Chave Final", "Classificação Geral"]
    ...
    # --- 1. FAZER PALPITES DE JOGOS (COM ABAS E TRAVAS) ---
    if menu == "Fazer Palpites de Jogos":
        ...
        if not jogos_db: 
            st.info("Nenhum jogo cadastrado nesta fase.")
        else:
            ...
            # Filtra considerando as travas globais do Super Admin
            jogos_abertos = []
            for j in jogos:
            ...
            if not jogos_abertos: 
                st.warning("🔒 Todos os jogos estão fechados ou bloqueados pelo Super Admin.")
            else:
                ...
                with aba_pendentes:
                    ...
                    if not jogos_faltando:
                        st.success("🎉 Parabéns! Todos os seus palpites para os jogos liberados estão em dia!")
                    else:
                        ...
                        with st.form("form_pendentes"):
                            ...
                            for j in jogos_faltando:
                                ...
                                v_classif = None
                                if is_mata:
                                    v_classif = st.radio("Quem passa de fase?", [j['time_casa'], j['time_fora']], key=f"p_cl_{j['id']}", horizontal=True)
                            ...
                            if st.form_submit_button("💾 Salvar Palpites Pendentes", use_container_width=True):
                                ...
                                st.success("Palpites pendentes guardados com sucesso!")
                                st.rerun()
                ...
                with aba_grupos:
                    ...
                    if not jogos_g: st.info("Nenhum jogo da fase de grupos aberto.")
                        ...
                        with st.form("form_grupos"):
                            st.caption("Pode preencher jogos de vários grupos e clicar em 'Salvar Todos' no fim da página.")
                            novos_p_g = {}
                            ...
                            if st.form_submit_button("💾 Salvar Todos os Grupos", use_container_width=True):
                                for id_j, dados in novos_p_g.items():
                                    if str(id_j) in mapa_meus: supabase.table("palpites_copa").update(dados).eq("email_usuario", st.session_state.email_usuario).eq("id_jogo", id_j).execute()
                                        ...
                                        dados.update({"email_usuario": st.session_state.email_usuario, "id_jogo": id_j})
                                        supabase.table("palpites_copa").insert(dados).execute()
                                st.success("Palpites da Fase de Grupos guardados!")
                                st.rerun()
                            ...
                            if st.form_submit_button("💾 Salvar Mata-Mata", use_container_width=True):
                                for id_j, dados in novos_p_m.items():
                                    if str(id_j) in mapa_meus: supabase.table("palpites_copa").update(dados).eq("email_usuario", st.session_state.email_usuario).eq("id_jogo", id_j).execute()
                                        ...
                                        dados.update({"email_usuario": st.session_state.email_usuario, "id_jogo": id_j})
                                        supabase.table("palpites_copa").insert(dados).execute()
                                st.success("Palpites do Mata-Mata guardados!")
                                st.rerun()

    # --- 2. BÔNUS 1: GRUPOS ---
    elif menu == "Bônus 1: Videntes dos Grupos":
        st.title("🔮 Videntes da Fase de Grupos")
        st.caption("1 pt por acerto + 2 pts extra se gabaritar o grupo inteiro.")
        ...
        with st.form("form_bonus_g"):
            ...
            for grp, times in GRUPOS_COPA.items():
                ...
                # Colunas ficam empilhadas no telemóvel para melhorar leitura
    ...
    # --- 3. BÔNUS 2: CHAVE DO MATA-MATA ---
        ...
        st.info("Preveja as seleções que vão avançar no Mata-Mata.")
        ...
        with st.form("form_chave_matamata"):
            ...
            if st.form_submit_button("💾 Salvar Árvore", use_container_width=True):
                if len(oitavas) != 16 or len(quartas) != 8 or len(semis) != 4 or len(finalistas) != 2:
                    ...
                    dados_chave = {
                        "oitavas": ",".join(oitavas), "quartas": ",".join(quartas),
                        "semis": ",".join(semis), "finalistas": ",".join(finalistas), "campeao": campeao
                    }
                    ...
                    st.success("Chave gravada! Boa sorte!")
    ...
    # --- 4. CLASSIFICAÇÃO GERAL ---
        ...
        if emails:
            ...
            if jogos_enc:
                ...
                for em, pts in df_comp.groupby('email')['pontos_jogos'].sum().to_dict().items(): 
            ...
            for u in usuarios_dados:
                ...
                rank_final.append({
                    "Nome": u['nome'], 
                    "Pontos Totais": total,
                    "Jogos": pontos_por_usuario[em]["Jogos"],
                    "Grupos": pontos_por_usuario[em]["Bónus 1"],
                    "Chave": pontos_por_usuario[em]["Bónus 2"]
                })
                
            df_final = pd.DataFrame(rank_final).sort_values("Pontos Totais", ascending=False).reset_index(drop=True)
            df_final.index += 1
            # st.dataframe permite scroll horizontal perfeito no mobile
    ...
    # --- 6. SUPER ADMIN GERAL ---
        ...
        with sa5:
            ...
            with st.form("form_gab_chave"):
                ...
                if st.form_submit_button("Gravar Realidade", use_container_width=True):
                    dados_g_chave = {
                        "id": 1, "oitavas": ",".join(oit), "quartas": ",".join(qua),
                        "semis": ",".join(sem), "finalistas": ",".join(fin), "campeao": camp
                    }
        ...
        with sa6:
            st.subheader("Travas e Configurações")
